...
namespace FirebaseEssentials.iOS
{
    public class FirebasePushNotificationManager : NSObject, IFirebasePushNotification, IUNUserNotificationCenterDelegate, IMessagingDelegate
	{
		...
		private static FirebasePushNotificationErrorEventHandler _onNotificationStatus;
		public event FirebasePushNotificationErrorEventHandler OnNotificationStatus {
			add {
				_onNotificationStatus += value;
			}
			remove {
				_onNotificationStatus -= value;
			}
		...
		public static bool _NotificationPermission;
        public bool NotificationPermission
		{
			get => _NotificationPermission;
			set
			{
		...
		public static void Initialize(NSDictionary options, bool autoRegistration = true)
		{
			...
			if (options?.ContainsKey(UIApplication.LaunchOptionsRemoteNotificationKey) ?? false) {
				...
				if (pushPayload != null) {
					...
					_onNotificationOpened?.Invoke(CrossFirebaseEssentials.Notifications, new FirebasePushNotificationResponseEventArgs(notificationResponse.Data, notificationResponse.Identifier, notificationResponse.Type));
		...
		public void RegisterForPushNotifications()
		{
			...
			Messaging.SharedInstance.Delegate = CrossFirebaseEssentials.Notifications as IMessagingDelegate;
			...
			if (UIDevice.CurrentDevice.CheckSystemVersion(10, 0)) {
				...
				UNUserNotificationCenter.Current.Delegate = CrossFirebaseEssentials.Notifications as IUNUserNotificationCenterDelegate;

				UNUserNotificationCenter.Current.RequestAuthorization(authOptions, (granted, error) => {
					if (error != null) {
                        _NotificationPermission = false;
						_onNotificationStatus?.Invoke(CrossFirebaseEssentials.Notifications, new FirebasePushNotificationErrorEventArgs(FirebasePushNotificationErrorType.PermissionDenied, error.Description));
					} else if (!granted) {
                        _NotificationPermission = false;
						_onNotificationStatus?.Invoke(CrossFirebaseEssentials.Notifications, new FirebasePushNotificationErrorEventArgs(FirebasePushNotificationErrorType.PermissionDenied, "Push notification permission not granted"));
					} else {
                        _NotificationPermission = true;
						_onNotificationStatus?.Invoke(CrossFirebaseEssentials.Notifications, new FirebasePushNotificationErrorEventArgs(FirebasePushNotificationErrorType.Success, "Push notification permission granted"));
		...
		public void UnregisterForPushNotifications()
		{
			...
			Installations.DefaultInstance.Delete((h) => { });
		}
		...
		public void WillPresentNotification(UNUserNotificationCenter center, UNNotification notification, Action<UNNotificationPresentationOptions> completionHandler)
		{
            ...
            if (parameters.TryGetValue(CrossFirebaseEssentials.StatusKey, out object status) && string.Compare(status.ToString(), CrossFirebaseEssentials.SyncToOwnersValue, true) == 0)
            {
                return;
            }
		...
		public static void DidReceiveMessage(NSDictionary data)
		{
			...
			CrossFirebaseEssentials.Notifications.NotificationHandler?.OnReceived(parameters);
			Debug.WriteLine("DidReceivedMessage");
		}
        ...
        {
			_NotificationPermission = false;
            _onNotificationStatus?.Invoke(CrossFirebaseEssentials.Notifications, new FirebasePushNotificationErrorEventArgs(FirebasePushNotificationErrorType.RegistrationFailed, error.Description));
		}
		...
		public void DidReceiveRegistrationToken(Messaging messaging, string fcmToken)
		{
			...
			// should be done.
			var refreshedToken = fcmToken != null ? fcmToken : string.Empty;

			if (!string.IsNullOrEmpty(refreshedToken)) {
				var isNewToken = refreshedToken != Token;
				_onTokenRefresh?.Invoke(CrossFirebaseEssentials.Notifications, new FirebasePushNotificationTokenEventArgs(refreshedToken, isNewToken));
				hasToken = true;
				...
				while (pendingTopics.Count > 0) {
					...
					if (pTopic != null) {
						if (pTopic.Item2) {
							CrossFirebaseEssentials.Notifications.Subscribe(pTopic.Item1);
						} else {
							CrossFirebaseEssentials.Notifications.Unsubscribe(pTopic.Item1);
						}
					}
				}
			}

			NSUserDefaults.StandardUserDefaults.SetString(refreshedToken, FirebaseTokenKey);
		}
		...
		{
			await Installations.DefaultInstance.GetInstallationIdAsync();
			var result = Messaging.SharedInstance.FcmToken;

			if (!string.IsNullOrEmpty(result)) {
				NSUserDefaults.StandardUserDefaults.SetString(result, FirebaseTokenKey);
			}
			return result;
		}
	}
