...
namespace MAUIEssentials.AppCode.Controls
{
    public class FlowLayout: Layout<View>
	{
		public static BindableProperty SpacingProperty =
			BindableProperty.Create(nameof(Spacing), typeof(Thickness), typeof(FlowLayout), new Thickness(6));
		...
		public class LayoutInfo
		{
			...
			public List<Rect> Bounds { get; private set; }
			...
			private List<Rect> SizeViews(IList<View> views, double widthConstraint)
			{
				var sizes = new List<Rect>();
				foreach (var view in views) {
					var sizeRequest = view.Measure(widthConstraint, double.PositiveInfinity);
					...
					sizes.Add(new Rect(0, 0, viewWidth, viewHeight));
				}
				return sizes;
			}

			private void LayoutViews(IList<View> views, List<Rect> sizes, double widthConstraint)
			{
				Bounds = new List<Rect>();
				_x = 0d;
				_y = 0d;
